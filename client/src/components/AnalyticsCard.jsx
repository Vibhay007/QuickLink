...
function AnalyticsCard({ analytics }) {
  ...
  return (
    <div className="analytics-grid">
      ...
      <style>{`
        .analytics-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 1rem; margin-bottom: 2rem; }
        .analytics-stat { text-align: center; padding: 1.25rem; }
        .analytics-value { display: block; font-size: 2rem; font-weight: 700; color: var(--primary); }
        .analytics-label { color: var(--text-muted); font-size: 0.9rem; }
        .analytics-top { grid-column: 1 / -1; }
        .analytics-top h3 { margin-bottom: 0.75rem; font-size: 1rem; }
        .analytics-top ul { list-style: none; }
        .top-link-item { display: flex; justify-content: space-between; align-items: center; padding: 0.65rem 0.5rem; border-bottom: 1px solid var(--border); }
        .top-link-item:last-child { border-bottom: none; }
        .link-details { display: flex; flex-direction: column; gap: 0.2rem; }
        .top-code { color: var(--primary); font-family: monospace; font-weight: 600; }
        .top-clicks { color: var(--text-muted); font-size: 0.85rem; }
        .btn-analytics { background: transparent; border: 1px solid var(--primary); color: var(--primary); padding: 0.4rem 0.8rem; border-radius: 20px; font-size: 0.8rem; font-weight: 500; cursor: pointer; transition: all 0.2s ease; }
        .btn-analytics:hover { background: var(--primary); color: #fff; }
        .btn-analytics.active { background: #64748b; border-color: #64748b; color: #fff; }
        .analytics-charts-panel { grid-column: 1 / -1; padding: 1.25rem; min-height: 100px; background: #fff; }
        .chart-loader, .chart-error-msg { text-align: center; color: var(--text-muted); padding: 1.5rem 0; font-size: 0.9rem; }
        .chart-error-msg { color: #ef4444; font-weight: 500; }
        .charts-layout h4 { margin: 0 0 1rem 0; font-size: 1rem; color: #334155; }
        .chart-wrapper { background: #fafafa; border: 1px solid #f1f5f9; border-radius: 10px; padding: 1rem; margin-bottom: 1rem; }
        .chart-wrapper h5 { margin: 0 0 0.75rem 0; font-size: 0.85rem; color: #64748b; }
        .charts-split-row { display: flex; gap: 1rem; flex-wrap: wrap; }
        .half-width { flex: 1; min-width: 240px; }
      `}</style>
    </div>
  );
}
